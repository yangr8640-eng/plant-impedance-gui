...
- `解析成功` 增加但看不到曲线：在该通道点击“重置缩放”，并检查测试时长、Y 格差设置是否合理。

### 6. 确认测试时长没有结束

每个通道的计时从该通道第一条有效电阻数据开始。
如果某个通道测试时长设置为 `24` 小时，到达 24 小时后该通道会自动停止继续追加曲线，其他通道不受影响。
