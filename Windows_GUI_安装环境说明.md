# 四路实时电阻曲线 GUI：Windows 安装环境说明

本文档用于说明把现有电脑上的“四路实时电阻曲线 GUI”复制到另一台 Windows 电脑时，需要安装和检查哪些环境。
...
只要能看到类似 `COM3`、`COM4`、`COM5`、`COM6` 的端口，GUI 才能在四个通道里分别选择并连接。
...
4. 打开 GUI 后点击任一通道里的“刷新串口”。
5. 顶部页签对应四个独立通道，页签名称会显示通道号、串口和植物编号。
6. 在每个通道里选择硬件对应的 `COM3`、`COM4`、`COM5`、`COM6` 等串口。
7. 每个通道选择和 SSCOM 或硬件程序一致的波特率，常用为 `115200`。
8. 开始测量前，每个通道单独填写“epoch 数量”和“每个 epoch 时长”，并选择秒、分钟或小时。例如 `4` 个 epoch、每个 `6` 小时，总测量时长为 24 小时。
9. 每个通道单独填写“横轴范围(h)”，例如 `6` 表示曲线显示最近 6 小时，`24` 表示显示完整 24 小时。
10. 依次切换四个页签并点击“连接并开始测量”。未选中的通道仍会继续在后台读取、绘图和自动记录 CSV。每个 epoch 结束时会自动生成一个以 `_epoch1.csv`、`_epoch2.csv` 等结尾的原始数据文件，直到所有 epoch 完成后自动停止。
11. 实验结束后，在当前页签点击“导出记录CSV”或“导出PDF”。导出只包含当前通道的数据，不会把四路合并到一个文件。

每个通道的自动记录文件会写入 `output` 文件夹，文件名包含通道号、串口号、植物编号和开始记录时间，例如：

```text
植物阻抗自动记录_通道1_COM3_绿萝01_20260707_153000_123_epoch1.csv
植物阻抗自动记录_通道1_COM3_绿萝01_20260707_153000_123_epoch2.csv
```
...
四路 GUI 内部也会阻止重复连接：如果通道 1 已连接 `COM3`，通道 2 再选择 `COM3` 并点击“连接”时会提示该串口已被其他通道连接。

### 4. 确认波特率一致

每个通道里的波特率必须和对应硬件程序一致，也要和 SSCOM 能正常显示数据时的波特率一致。
例如硬件是 `115200`，对应通道也必须选择 `115200`。
...
- `收到字节` 一直是 0：该通道没收到硬件数据。优先检查串口号、驱动、USB 线、硬件供电、硬件是否主动发送。
- `收到字节` 增加，但 `解析成功` 是 0：该通道收到了文本，但没有识别出电阻值。请把接收日志中的原始数据格式发给开发者。
- `解析成功` 增加但看不到曲线：在该通道点击“重置缩放”，并检查 epoch 数量、epoch 时长、Y 格差设置是否合理。

### 6. 确认 epoch 测量还没有全部结束

每个通道的计时从串口连接成功并开始测量时开始，不等待第一条有效电阻数据。
每个 epoch 到时后，该通道会保存当前 epoch 的 CSV 并自动进入下一个 epoch。所有 epoch 完成后，该通道才会自动停止继续追加曲线，其他通道不受影响。
...
5. 对应硬件的 USB 转串口驱动
6. 设备管理器中能看到 COM 端口
7. 四个通道分别选择不同 COM 口，不能重复连接同一个串口
8. 每个通道的波特率与对应硬件一致
9. 硬件每条数据按 R=1023.5 ohm 这类格式发送，并带换行
```
